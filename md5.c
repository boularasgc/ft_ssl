...
/*   Updated: 2019/10/21 21:13:13 by mboulara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			construct_mssg(t_cx data, t_ctx *h)
{
	...
	if (h->ini_len % 64 < 56)
		h->msg = (uint8_t *)malloc(((h->ini_len / 64) + 1) * 64);
	else
		h->msg = (uint8_t *)malloc(((h->ini_len / 64) + 2) * 64);
	ft_bzero(h->msg, h->newlen);
	ft_memcpy(h->msg, data.line, h->ini_len);
	h->msg[h->ini_len] = 128;
	bit_len = h->ini_len * 8;
	ft_memcpy(h->msg + h->newlen, &bit_len, 4);
}
...
void			md5_main_loop(t_ctx *h, u_int32_t j)
{
	...
	h->d = h->c;
	h->c = h->b;
	h->b += LEFTROTATE(h->f + h->a + g_k_table[j] + h->w[h->g], g_s_table[j]);
	h->a = h->temp;
}
