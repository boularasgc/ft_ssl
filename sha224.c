...
/*   Updated: 2019/10/22 01:37:45 by mboulara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			copy_mesg_224_w(t_224ctx *h, size_t l)
{
	...
	while (i < 64)
	{
		h->w[i] = WJ1(h->w[i - 2]) + h->w[i - 7];
		h->w[i] += WJ0(h->w[i - 15]) + h->w[i - 16];
		i++;
	}
}
...
void			compression_224loop(t_224ctx *h)
{
	...
	while (i < 64)
	{
		t1 = h->h + EP1(h->e) + CH(h->e, h->f, h->g) + g_k[i] + h->w[i];
		t2 = EP0(h->a) + MAJ(h->a, h->b, h->c);
		h->h = h->g;
		h->g = h->f;
		h->f = h->e;
		h->e = h->d + t1;
